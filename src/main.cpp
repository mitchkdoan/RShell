...
int main() {
    ...
    while (input != "exit") {
        parse_string(commands, connectors, input);
        
        if (commands.size() == 1) { // single command in command line
            Base* s_obj = single_command(commands, connectors); // create executables here
            s_obj->execute();
        }
        else {
            Base* d_obj;
            while (!commands.empty()) {
                d_obj = double_command(prev_obj, commands, connectors, flag, p_counter); // create executables here
            }
            // tests for equal # of parenthesis before execute
            if (p_counter != 0) {
